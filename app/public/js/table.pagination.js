...
var table = $("#table-sortable").tableSortable({
    ...
    formatCell: function (row, key) {
        if (key == "name") {
            elemNum = elemNum + 1;
            return $("<span></span>")
                .attr("id", (elemNum - 1) % 20)
                .attr("name", "clickable-cell")
                .text(row[key]);
        }
        return row[key];
    },
...
$.get(url, function (data) {
    ...
    for (let i = 0; i < Math.min(data.length, 3000); i++) {
        L.marker([data[i].reclat, data[i].reclong], { icon: meteorIcon })
            .addTo(layerGroup)
            .bindPopup(
                data[i].name +
                    "<br />Класс: " +
    ...
    document.getElementsByName("clickable-cell").forEach((item) => {
        item.addEventListener("click", (event) => {
            ...
            if (!table.getCurrentPageData()[rowNumOnPage].reclat) {
                ...
                map.setView(
                    [
                        table.getCurrentPageData()[rowNumOnPage].reclat,
                        table.getCurrentPageData()[rowNumOnPage].reclong,
                    ],
                    8
                );
            }
